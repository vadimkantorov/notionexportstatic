...
def richtext_tohtml(block, ctx = {}, title_mode = False, caption = False, rich_text = False):
    ...
    if isinstance(block, list):
        return ''.join(richtext_tohtml(subblock, ctx, title_mode = title_mode) for subblock in block).strip()
    ...
    if block['type'] == 'mention':
        return mention_tohtml(block, ctx)
    if block['type'] == 'equation':
        return equation_tohtml(block, ctx, inline = True)
    
    res = html.escape(plain_text)
    if href:
        res = link_to_page_tohtml(block, ctx, line_break = False) if href.startswith('/') else linklike_tohtml(block, ctx)
    if anno['bold']:
        res = f'<b>{res}</b>' 
    if anno['italic']:
        res = f'<i>{res}</i>'
...
def blocktag_tohtml(block = {}, ctx = {}, class_name = '', tag = '', selfclose = False, set_html_contents_and_close = None, prefix = '', suffix = '', attrs = {}, **kwargs):
...
def textlike_tohtml(block, ctx, tag = 'span', class_name = '', attrs = {}, icon = '', checked = None):
    ...
    rich_text = richtext_tohtml(block, ctx, rich_text = True)
    checkbox = '<input type="checkbox" disabled {} />'.format('checked' * checked) if checked is not None else ''
    html = checkbox + rich_text + childrenlike_tohtml(block, ctx) + icon
    return blocktag_tohtml(block, ctx, tag = tag, class_name = class_name + f' notion-color-{color}', attrs = attrs, set_html_contents_and_close = html)

def toggle_tohtml(block, ctx, tag = 'span', class_name = 'notion-toggle-block', attrs = {}, icon = ''):
    color = block.get(get_block_type(block), {}).get('color', '')
    rich_text = richtext_tohtml(block, ctx, rich_text = True)
    return blocktag_tohtml(block, ctx, tag = 'details', class_name = f'notion-color-{color} notion-toggle-like ' + class_name, attrs = dict(attrs, open = None) if ctx['html_details_open'] else attrs, set_html_contents_and_close = f'<summary><{tag}>{rich_text}{icon}</{tag}></summary>\n' + childrenlike_tohtml(block, ctx))

def headinglike_tohtml(block, ctx, tag, class_name = ''):
    ...
    return (textlike_tohtml if not is_toggleable else toggle_tohtml)(block, ctx, tag = tag, class_name = 'notion-heading-like ' + class_name, attrs = dict(id = block_id_no_dashes), icon = anchor)

def linklike_tohtml(block, ctx, tag = 'a', class_name = '', full_url_as_caption = False, icon = '', line_break = False, download = None):
...
def icon_image_tohtml(icon_url):
    return f'<img src="{icon_url}"></img>'
...
def table_of_contents_tohtml(block, ctx, tag = 'ul', class_name = 'notion-table_of_contents-block'):
    # https://www.notion.so/help/columns-headings-and-dividers#how-it-works
    if block.get('site_table_of_contents_page_ids'):
        table_of_contents_page_tree = lambda page_ids: '' if not page_ids else ('<ul class="notion-table_of_contents-site-page-list">\n' + '\n'.join('<li class="notion-table_of_contents-site-page-item">\n{link_to_page}\n{child_pages}\n</li>'.format(link_to_page = link_to_page_tohtml(dict(type = 'link_to_page', link_to_page = dict(type = 'page_id', page_id = page_id)), ctx, line_break = False), child_pages = table_of_contents_page_tree([ get_block_id(page) for page in ctx['child_pages_by_parent_id'].get(page_id, []) ]) ) for page_id in page_ids) + '\n</ul>')
    ...
    if block.get('site_table_of_contents_flat_page_ids'):
        table_of_contents_page_tree = lambda page_ids: '' if not page_ids else '\n'.join('<li class="notion-table_of_contents-site-page-item">{link_to_page}</li>\n{child_pages}'.format(link_to_page = link_to_page_tohtml(dict(type = 'link_to_page', link_to_page = dict(type = 'page_id', page_id = page_id)), ctx, line_break = False, class_name = 'page-link'), child_pages = table_of_contents_page_tree([ get_block_id(page) for page in ctx['child_pages_by_parent_id'].get(page_id, []) ]) ) for page_id in page_ids)
    ...
    for block in headings:
        ...
        rich_text = richtext_tohtml(block, ctx, rich_text = True, title_mode = True)
    ...
    return blocktag_tohtml(block, ctx, tag = tag, class_name = class_name + f' notion-color-{color}', set_html_contents_and_close = children, prefix = '<hr/>', suffix = '<hr/>')

def table_tohtml(block, ctx, tag = 'table', class_name = 'notion-table-block'):
    ...
    for i, subblock in enumerate(rows):
        ...
        for j, cell in enumerate(cells):
            ...
            children += f'<{tag_cell}>' + (''.join('<div>{rich_text}</div>'.format(rich_text = richtext_tohtml(subcell, ctx)) for subcell in cell) if isinstance(cell, list) else richtext_tohtml(cell, ctx)) + f'</{tag_cell}>\n'
        ...
        if i == 0:
            children += '\n</thead>\n<tbody>\n' if has_row_header else ''
    children += '\n</tbody>\n'
    return blocktag_tohtml(block, ctx, tag = tag, class_name = class_name, set_html_contents_and_close = children)

def video_tohtml(block, ctx, tag = 'p', class_name = 'notion-video-block'):
    url = get_asset_url(block, ctx)
    is_youtube, url, urlimg = normalize_youtube_url(url, embed = True)
    caption = richtext_tohtml(block, ctx, caption = True)
    ...
    return blocktag_tohtml(block, ctx, tag = tag, class_name = class_name, set_html_contents_and_close = html_contents)

def image_tohtml(block, ctx, tag = 'img', class_name = 'notion-image-block'):
    return blocktag_tohtml(block, ctx, tag = 'figure', class_name = class_name, set_html_contents_and_close = '<{tag} src="{src}" alt="{rich_text_alt}"></{tag}><figcaption>{rich_text}</figcaption>'.format(tag = tag, src = get_asset_url(block, ctx), rich_text_alt = richtext_tohtml(block, ctx, caption = True, title_mode = True), rich_text = richtext_tohtml(block, ctx, caption = True, title_mode = False)))

def embed_tohtml(block, ctx, tag = 'iframe', class_name = 'notion-embed-block', caption = '', attrs = dict(width = 640, height = 480)):
    block_type = get_block_type(block)
    src = get_block_url(block) or block.get(block_type, {}).get(block.get(block_type, {}).get('type', ''), {}).get('url') or ''
    src = get_asset_url(src, ctx)
    rich_text = caption or richtext_tohtml(block, ctx, caption = True) 
    attrstr = ' '.join(f'{k}="{v}"' if v is not None else k for k, v in attrs.items())
    return blocktag_tohtml(block, ctx, tag = 'figure', class_name = class_name, set_html_contents_and_close = f'<figcaption>{rich_text}</figcaption><{tag} src="{src}" {attrstr}></{tag}>')

def bookmark_tohtml(block, ctx, tag = 'a', class_name = 'notion-bookmark-block'):
    src = get_block_url(block)
    rich_text = richtext_tohtml(block, ctx, caption = True) or html.escape(block.get(get_block_type(block), {}).get('name') or block.get('plain_text') or src)
    try:
        netloc = urllib.parse.urlparse(src).netloc
    except:
        netloc = ''
    return blocktag_tohtml(block, ctx, tag = tag, attrs = dict(href = src), class_name = class_name, set_html_contents_and_close = f'<figure>{netloc}<br/><figcaption>{rich_text}</figcaption></figure>')

def mention_tohtml(block, ctx, tag = 'div', class_name = dict(page = 'notion-page-mention-token', database = 'notion-database-mention-token', link_preview = 'notion-link-mention-token', user = 'notion-user-mention-token', date = 'notion-date-mention-token'), untitled = 'Untitled'):
    mention_type = block['mention'].get('type')
    mention_payload = block['mention'][mention_type]
    if mention_type == 'page':
        return link_to_page_tohtml(dict(block, type = 'link_to_page', link_to_page = dict(type = 'page_id', page_id = mention_payload.get('id', ''))), ctx, class_name = class_name[mention_type])
    if mention_type == 'database':
        return linklike_tohtml(block, ctx, class_name = class_name[mention_type])
    if mention_type == 'link_preview':
        return link_preview_tohtml(block, ctx, class_name = class_name[mention_type])
    if mention_type == 'user':
        return blocktag_tohtml(block, ctx, tag = 'strong', class_name = class_name[mention_type], set_html_contents_and_close = '@{user_name}#{user_id}'.format(user_id = mention_payload.get('id', ''), user_name = block.get('plain_text', '').removeprefix('@')))
    if mention_type == 'date':
        return blocktag_tohtml(block, ctx, tag = 'strong', class_name = class_name[mention_type], set_html_contents_and_close = '@{date_text}'.format(date_text = html.escape(block.get('plain_text', ''))))
    return unsupported_tohtml(block, ctx)

def link_to_page_tohtml(block, ctx, tag = 'a', caption = None, line_break = True, class_name = 'notion-alias-block', icon = '', untitled = '???'):
    ...
    return blocktag_tohtml(block, ctx, tag = tag, attrs = dict(href = link_to_page_info['href']), class_name = class_name, set_html_contents_and_close = caption) + '<br/>' * line_break

def bulleted_list_item_tohtml(block, ctx, tag = 'ul', begin = False, end = False, class_name = 'notion-bulleted_list-block'):
    return f'<{tag} class="{class_name}">\n' * begin + textlike_tohtml(block, ctx, tag = 'li') + f'\n</{tag}>\n' * end

def numbered_list_item_tohtml(block, ctx, tag = 'ol', begin = False, end = False, class_name = 'notion-numbered_list-block'):
    return f'<{tag} class="{class_name}">\n' * begin + textlike_tohtml(block, ctx, tag = 'li') + f'\n</{tag}>\n' * end 

def to_do_tohtml(block, ctx, tag = 'div', class_name = 'notion-to_do-block', begin = False, end = False):
    return textlike_tohtml(block, ctx, tag = tag, class_name = class_name, checked = block.get(get_block_type(block), {}).get('checked', False))

def code_tohtml(block, ctx, tag = 'code', class_name = 'notion-code-block'):
    return blocktag_tohtml(block, ctx, tag = 'figure', attrs = {'data-language' : block['code'].get('language', '')}, class_name = class_name, set_html_contents_and_close = '<figcaption>{caption}</figcaption>\n<pre><{tag} class="language-{language}">\n'.format(language = block['code'].get('language', ''), caption = richtext_tohtml(block, ctx, caption = True), tag = tag) + richtext_tohtml(block, ctx, rich_text = True) + f'\n</{tag}></pre>')

def synced_block_tohtml(block, ctx, tag = 'figure', class_name = 'notion-synced_block-block'):
    return blocktag_tohtml(block, ctx, tag = tag, class_name = class_name, set_html_contents_and_close = '<figcaption>{synced_from_block_id}</figcaption>\n{children}'.format(synced_from_block_id = block['synced_block'].get('synced_from', {}).get('block_id', ''), children = childrenlike_tohtml(block, ctx)))

def equation_tohtml(block, ctx, tag = 'code', class_name = 'notion-equation-block', inline = False):
    return blocktag_tohtml(block, ctx, tag = tag, class_name = class_name if not inline else class_name.replace('block', 'inline'), set_html_contents_and_close = html.escape(block['equation'].get('expression', '') or block.get('plain_text', '')))

def pdf_tohtml(block, ctx, tag = 'a', class_name = 'notion-pdf-block'):
    return embed_tohtml(block, ctx, class_name = class_name, caption = linklike_tohtml({k : v for k, v in block.items() if k != 'id'}, ctx, tag = tag) if not is_url_datauri(get_asset_url(block, ctx)) else '<div><b>{caption}</b></div>'.format(caption = html.escape(get_url_basename(get_block_url(block)))) )

def file_tohtml(block, ctx, tag = 'a', class_name = 'notion-file-block'):
    return linklike_tohtml(block, ctx, tag = tag, class_name = class_name, line_break = True)

def breadcrumb_tohtml(block, ctx, tag = 'div', class_name = 'notion-breadcrumb-block', sep = '&nbsp;/&nbsp;'):
    return blocktag_tohtml(block, ctx, tag = tag, class_name = class_name, set_html_contents_and_close = sep.join(link_to_page_tohtml(subblock, ctx, line_break = False) for subblock in reversed(ctx['page_parent_paths'][get_block_id(get_page_current(block, ctx)) ])))

def template_tohtml(block, ctx, tag = 'figure', class_name = 'notion-template-block'):
    return blocktag_tohtml(block, ctx, tag = tag, class_name = class_name, set_html_contents_and_close = '<figcaption>{rich_text}</figcaption>\n{children}'.format(rich_text = richtext_tohtml(block, ctx, rich_text = True), children = childrenlike_tohtml(block, ctx)))

def child_database_tohtml(block, ctx, tag = 'figure', class_name = 'notion-child_database-block', untitled = '???'):
    return blocktag_tohtml(block, ctx, tag = tag, class_name = class_name, set_html_contents_and_close = '<figcaption><strong>{child_database_title}</strong></figcaption>'.format(child_database_title = html.escape(block['child_database'].get('title') or untitled)))
...
def link_preview_tohtml(block, ctx, tag = 'a', class_name = 'notion-link_preview-block'):
    return linklike_tohtml(block, ctx, tag = tag, class_name = class_name, line_break = True)

def quote_tohtml(block, ctx, tag = 'blockquote', class_name = 'notion-quote-block'):
    return textlike_tohtml(block, ctx, tag = tag, class_name = class_name)

def child_page_tohtml(block, ctx, tag = 'article', class_name = 'notion-page-block', **kwargs):
    return page_tohtml(block, ctx, tag = tag, class_name = class_name, **kwargs)

def unsupported_tohtml(block, ctx, tag = 'br', class_name = 'notion-unsupported-block', **ignored):
    return blocktag_tohtml(block, ctx, tag = tag, class_name = class_name, attrs = {'data-notion-block_type' : get_block_type(block)}, selfclose = True)

def divider_tohtml(block, ctx, tag = 'hr', class_name = 'notion-divider-block'):
    return blocktag_tohtml(block, ctx, tag = tag, class_name = class_name, selfclose = True)

def heading_1_tohtml(block, ctx, tag = 'h1', class_name = 'notion-header-block'):
    return headinglike_tohtml(block, ctx, tag = tag, class_name = class_name)

def heading_2_tohtml(block, ctx, tag = 'h2', class_name = 'notion-sub_header-block'):
    return headinglike_tohtml(block, ctx, tag = tag, class_name = class_name)

def heading_3_tohtml(block, ctx, tag = 'h3', class_name = 'notion-sub_sub_header-block'):
    return headinglike_tohtml(block, ctx, tag = tag, class_name = class_name)

def paragraph_tohtml(block, ctx, tag = 'p', class_name = 'notion-text-block'):
    return blocktag_tohtml(block, ctx, tag = 'br', class_name = class_name, selfclose = True) if paragraph_is_empty(block, ctx) else textlike_tohtml(block, ctx, tag = tag, class_name = class_name)

def column_list_tohtml(block, ctx, tag = 'div', class_name = 'notion-column_list-block'):
    return blocktag_tohtml(block, ctx, tag = tag, class_name = class_name + ' notion_column_list-block-vertical' * (ctx['html_columnlist_disable'] is True), set_html_contents_and_close = childrenlike_tohtml(block, ctx))

def column_tohtml(block, ctx, tag = 'div', class_name = 'notion-column-block'):
    return blocktag_tohtml(block, ctx, tag = tag, class_name = class_name, set_html_contents_and_close = childrenlike_tohtml(block, ctx, tag = tag)) 


##############################

def childrenlike_tomarkdown(block, ctx, tag = '', newline = '\n'):
    return newline.join(tag + block_tomarkdown(subblock, ctx) for i, subblock in enumerate(sum([block.get(key, []) or block.get(block.get('type'), {}).get(key, []) for key in ('children', 'blocks')], []))) + newline

def richtext_tomarkdown(block, ctx, title_mode = False, caption = False, rich_text = False):
    if isinstance(block, dict):
        block_type = get_block_type(block)
        if rich_text:
            block = get_block_rich_text(block)
        if caption: 
            block = block.get(get_block_type(block), {}).get('caption', [])
    if isinstance(block, list):
        return ''.join(richtext_tomarkdown(subblock, ctx, title_mode = title_mode) for subblock in block).strip()
    ...
    if title_mode:
        return plain_text
    if block['type'] == 'mention':
        return mention_tomarkdown(block, ctx)
    if block['type'] == 'equation':
        return equation_tomarkdown(block, ctx, inline = True)
    res = (plain_text)
    if href:
        res = link_to_page_tomarkdown(block, ctx, line_break = False) if href.startswith('/') else linklike_tomarkdown(block, ctx)
...
def toggle_tomarkdown(block, ctx, tag = '', icon = '', title_mode = False):
...
def icon_image_tomarkdown(icon_url):
    return f'![cover]({icon_url})'
...
def headinglike_tomarkdown(block, ctx, tag = ''):
    block_id_no_dashes = get_block_id_no_dashes(block)
    block_slug = get_heading_slug(block, ctx)
    is_toggleable = block.get(get_block_type(block), {}).get('is_toggleable')
    block_hash = block_slug if ctx['extract_mode'] == 'flat.md' else block_id_no_dashes
    anchor = f' [#](#{block_hash})'
    return (textlike_tomarkdown if not is_toggleable else toggle_tomarkdown)(block, ctx, tag = f'<i id="{block_id_no_dashes}"></i><i id="{block_slug}"></i>\n' + tag, icon = anchor)

def linklike_tomarkdown(block, ctx, tag = '', full_url_as_caption = True, line_break = False):
    src = get_block_url(block)
    asset_url = get_asset_url(block, ctx)
    rich_text = richtext_tomarkdown(block, ctx, caption = True) or (block.get(get_block_type(block), {}).get('name') or block.get('plain_text') or (src if full_url_as_caption else get_url_basename(src)))
    return (linklike_tohtml({k : v for k, v in block.items() if k != 'id'}, ctx) if is_url_datauri(asset_url) else f'[{tag}{rich_text}]({src})') + '\n\n' * line_break

def table_tomarkdown(block, ctx, sanitize_table_cell = lambda md: md.replace('\n', ' ')):
    table_width = block['table'].get('table_width', 0)
    has_row_header = block['table'].get('has_row_header', False)
    has_column_header = block['table'].get('has_column_header', False)
    rows = get_block_children(block)
    ...
    for i, subblock in enumerate(rows):
        cells = subblock.get('table_row', {}).get('cells', [])
        children += ' | ' + ' | '.join(sanitize_table_cell(richtext_tomarkdown(cell, ctx)) for cell in cells) + ' | ' + '\n'
        if i == 0:
            children += ' | ' + ' | '.join(['----'] * len(cells)) + ' | ' + '\n'
    children += '\n'
    return children

def table_of_contents_tomarkdown(block, ctx, tag = '* '):
    if block.get('site_table_of_contents_page_ids'):
        table_of_contents_page_tree = lambda page_ids, depth = 0: '' if not page_ids else ''.join(depth * 4 * ' ' + '{tag}{link_to_page}\n{child_pages}'.format(tag = tag, link_to_page = link_to_page_tomarkdown(dict(type = 'link_to_page', link_to_page = dict(type = 'page_id', page_id = page_id)), ctx, line_break = False), child_pages = table_of_contents_page_tree([get_block_id(page) for page in ctx['child_pages_by_parent_id'].get(page_id, [])], depth + 1)) for page_id in page_ids)
        page_ids = block.get('site_table_of_contents_page_ids', [])
        child_page_ids = set(get_block_id(child_page) for child_pages in ctx['child_pages_by_parent_id'].values() for child_page in child_pages)
        root_page_ids = [page_id for page_id in page_ids if page_id not in child_page_ids]
    ...
    for block in headings:
        ...
        heading_type = get_block_type(block)
        nominal_heading_type, effective_heading_type = heading_type, min(heading_type, inc_heading_type(effective_heading_type) if heading_type > nominal_heading_type else effective_heading_type)
        rich_text = richtext_tomarkdown(block, ctx, rich_text = True, title_mode = True)
        children += max(0, heading_type2depth[effective_heading_type] - 1) * 4 * ' ' + f'{tag}[{rich_text}](#{block_hash})\n'
    children += '---\n'
    return children

def mention_tomarkdown(block, ctx):
    mention_type = block['mention'].get('type')
    mention_payload = block['mention'][mention_type]
    if mention_type == 'page':
        return link_to_page_tomarkdown(dict(block, type = 'link_to_page', link_to_page = dict(type = 'page_id', page_id = mention_payload.get('id', ''))), ctx, line_break = False)
    if mention_type == 'database':
        return linklike_tomarkdown(block, ctx)
    if mention_type == 'link_preview':
        return link_preview_tomarkdown(block, ctx)
    if mention_type == 'user':
    ...
    return unsupported_tomarkdown(block, ctx)

def page_tomarkdown(block, ctx, strftime = '%Y/%m/%d %H:%M:%S'):
    datetime_published = get_page_time_published(block, ctx, strftime = strftime, key = 'unix_seconds_generated' if ctx['timestamp_published'] else 'unix_seconds_downloaded') 
    src_cover = get_asset_url(get_page_cover_url(block), ctx)
    page_id = get_block_id(block)
    page_id_no_dashes = get_block_id_no_dashes(block)
...
def unsupported_tomarkdown(block, ctx, tag = '*'):
    return ' {tag}unsupported notion block [{block_id}, {block_type}]{tag} '.format(tag = tag, block_id = get_block_id(block), block_type = get_block_type(block))

def synced_block_tomarkdown(block, ctx):
    return '---\n**{synced_from_block_id}**\n{children}\n---'.format(synced_from_block_id = block['synced_block'].get('synced_from', {}).get('block_id', ''), children = childrenlike_tomarkdown(block, ctx))

def equation_tomarkdown(block, ctx, inline = False):
    return ('```math\n' if not inline else '`' ) + (block['equation'].get('expression', '') or block.get('plain_text', '')) + ('\n```' if not inline else '`')

def child_database_tomarkdown(block, ctx, untitled = '???', tag = '**'):
    return ' {tag}{child_database_title}{tag} '.format(tag = tag, child_database_title = (block['child_database'].get('title') or untitled))

def template_tomarkdown(block, ctx):
    return '---\n{rich_text}\n{children}\n---'.format(rich_text = richtext_tomarkdown(block, ctx, rich_text = True), children = childrenlike_tomarkdown(block, ctx))

def breadcrumb_tomarkdown(block, ctx, sep = ' / '):
    return sep.join(link_to_page_tomarkdown(subblock, ctx, line_break = False) for subblock in reversed(ctx['page_parent_paths'][get_block_id(get_page_current(block, ctx))]))

def code_tomarkdown(block, ctx):
...
def link_to_page_tomarkdown(block, ctx, line_break = True, caption = None):
    link_to_page_info = get_page_link_info(block, ctx)
...
def child_page_tomarkdown(block, ctx):
    return page_tomarkdown(block, ctx)

def divider_tomarkdown(block, ctx, tag = '---'):
    return tag

def heading_1_tomarkdown(block, ctx, tag = '# '  ):
    return headinglike_tomarkdown(block, ctx, tag = tag) 

def heading_2_tomarkdown(block, ctx, tag = '## ' ):
    return headinglike_tomarkdown(block, ctx, tag = tag) 

def heading_3_tomarkdown(block, ctx, tag = '### '):
    return headinglike_tomarkdown(block, ctx, tag = tag) 

def paragraph_tomarkdown(block, ctx):
    return '\n\n' if paragraph_is_empty(block, ctx) else textlike_tomarkdown(block, ctx)

def column_list_tomarkdown(block, ctx):
    return childrenlike_tomarkdown(block, ctx)

def column_tomarkdown(block, ctx):
    return childrenlike_tomarkdown(block, ctx)

def bulleted_list_item_tomarkdown(block, ctx, tag = '* ' , begin = False, end = False):
    return '\n' * begin + tag + textlike_tomarkdown(block, ctx) + '\n' * end

def numbered_list_item_tomarkdown(block, ctx, tag = '1. ', begin = False, end = False):
    return '\n' * begin + tag + textlike_tomarkdown(block, ctx) + '\n' * end

def to_do_tomarkdown(block, ctx, tag = '- ' , begin = False, end = False):
    return '\n' * begin + textlike_tomarkdown(block, ctx, tag = tag, checked = block.get(get_block_type(block), {}).get('checked', False)) + '\n' * end

def file_tomarkdown(block, ctx, tag = '📎'):
    return linklike_tomarkdown(block, ctx, tag = tag, line_break = True)

def pdf_tomarkdown(block, ctx, tag = '📄'):
    return linklike_tomarkdown(block, ctx, tag = tag, line_break = True)
...
def embed_tomarkdown(block, ctx):
    return embed_tohtml(block, ctx)

def callout_tomarkdown(block, ctx):
    return quote_tomarkdown(block, ctx, icon = get_callout_icon(block, ctx))
...
def get_block_rich_text(block): 
    return block.get('rich_text', []) or block.get('text', []) or block.get(get_block_type(block), {}).get('rich_text', []) or block.get(get_block_type(block), {}).get('text', []) or []

def paragraph_is_empty(block, ctx):
    return block.get('has_children') is False and not get_plain_text(get_block_rich_text(block)).strip()
...
def get_heading_slug(block, ctx, space = '-', lower = True, prefix = ''):
    s = richtext_tohtml(block, ctx, rich_text = True, title_mode = True)
...
def block2(block, ctx = {}, render_block_registry = {}, render_block_registry_with_begin_end = {}, begin = False, end = False, newline = '', **kwargs):
    # https://developers.notion.com/reference/block
    block_type = get_block_type(block)
    if block_type in render_block_registry_with_begin_end:
        return render_block_registry_with_begin_end[block_type](block, ctx, begin = begin, end = end, **kwargs) + newline
    if block_type not in render_block_registry or block_type == 'unsupported':
    ...
    return render_block_registry[block_type](block, ctx, **kwargs) + newline
...
def extractall(output_path, ctx, theme, page_ids = [], notion_pages = {}, notion_pages_flat = {},  snippets = {}):
    ...
    if ctx['extract_mode'] in extract_mode_single:
        ...
        if ext == '.md':
           notionstr = theme.sitepages_tomarkdown(page_ids, ctx = dict(ctx, assets = notion_assets_for_blocks, meta = meta), notion_pages = notion_pages_flat, render_block = block_tomarkdown, snippets = snippets)
        if ext == '.html':
           notionstr = theme.sitepages_tohtml(page_ids, ctx = dict(ctx, assets = notion_assets_for_blocks, meta = meta), notion_pages = notion_pages_flat, render_block = block_tohtml, snippets = snippets)
        if ext == '.json':
            notionjson = dict(
...
render_block_registry_tohtml = dict(
    bookmark = bookmark_tohtml,
    breadcrumb = breadcrumb_tohtml,
    bulleted_list_item = bulleted_list_item_tohtml,
    callout = callout_tohtml,
    child_database = child_database_tohtml,
    child_page = child_page_tohtml,
    code = code_tohtml,
    column_list = column_list_tohtml, column = column_tohtml,
    divider = divider_tohtml,
    embed = embed_tohtml,
    equation = equation_tohtml,
    file = file_tohtml,
    heading_1 = heading_1_tohtml, heading_2 = heading_2_tohtml, heading_3 = heading_3_tohtml,
    image = image_tohtml,
    link_preview = link_preview_tohtml,
    mention = mention_tohtml,
    numbered_list_item = numbered_list_item_tohtml,
    paragraph = paragraph_tohtml,
    pdf = pdf_tohtml,
    quote = quote_tohtml,
    synced_block = synced_block_tohtml,
    table = table_tohtml,
    table_of_contents = table_of_contents_tohtml,
    template = template_tohtml,
    to_do = to_do_tohtml,
    toggle = toggle_tohtml,
    video = video_tohtml,

    link_to_page = link_to_page_tohtml,
    page = page_tohtml,
    unsupported = unsupported_tohtml,
)
render_block_registry_with_begin_end_tohtml = dict(
    numbered_list_item = numbered_list_item_tohtml,
    bulleted_list_item = bulleted_list_item_tohtml,
    to_do = to_do_tohtml
)

render_block_registry_tomarkdown = dict(
    bookmark = bookmark_tomarkdown, 
    breadcrumb = breadcrumb_tomarkdown,
    bulleted_list_item = bulleted_list_item_tomarkdown, 
    callout = callout_tomarkdown, 
    child_database = child_database_tomarkdown,
    child_page = child_page_tomarkdown,
    code = code_tomarkdown, 
    column_list = column_list_tomarkdown, column = column_tomarkdown, 
    divider = divider_tomarkdown, 
    embed = embed_tomarkdown, 
    equation = equation_tomarkdown, 
    file = file_tomarkdown, 
    heading_1 = heading_1_tomarkdown, heading_2 = heading_2_tomarkdown, heading_3 = heading_3_tomarkdown, 
    image = image_tomarkdown,
    link_preview = link_preview_tomarkdown,
    mention = mention_tomarkdown,
    numbered_list_item = numbered_list_item_tomarkdown, 
    paragraph = paragraph_tomarkdown, 
    pdf = pdf_tomarkdown, 
    quote = quote_tomarkdown, 
    synced_block = synced_block_tomarkdown,
    table = table_tomarkdown, 
    table_of_contents = table_of_contents_tomarkdown, 
    template = template_tomarkdown,
    to_do = to_do_tomarkdown, 
    toggle = toggle_tomarkdown, 
    video = video_tomarkdown, 
    
    link_to_page = link_to_page_tomarkdown, 
    page = page_tomarkdown,
    unsupported = unsupported_tomarkdown,
)
render_block_registry_with_begin_end_tomarkdown = dict(
    numbered_list_item = numbered_list_item_tomarkdown,
    bulleted_list_item = bulleted_list_item_tomarkdown,
    to_do = to_do_tomarkdown
)

def block_tohtml(block, ctx = {}, begin = False, end = False, **kwargs):
    return block2(block, ctx, render_block_registry = render_block_registry_tohtml, render_block_registry_with_begin_end = render_block_registry_with_begin_end_tohtml, begin = begin, end = end, **kwargs)

def block_tomarkdown(block, ctx = {}, begin = False, end = False, **kwargs):
    return block2(block, ctx, render_block_registry = render_block_registry_tomarkdown, render_block_registry_with_begin_end = render_block_registry_with_begin_end_tomarkdown, begin = begin, end = end, **kwargs)
...
class minima:
    ...
    def sitepages_tohtml(page_ids = [], ctx = {}, notion_pages = {}, render_block = (lambda page, ctx, **kwargs: ''), snippets = {}):
        snippets = minima.snippets_default | snippets
        ...
        emoji_svg = snippets.get('emoji_svg', '')
    ...
    .notion-page-like-edit-icon::after { content: "✏️" }
    ...
    .notion-page-like-icon, .notion-heading-like-icon { visibility : hidden; }
    .notion-page-block:hover > .notion-page-like-icon, .notion-heading-like:hover > .notion-heading-like-icon { visibility : visible !important; }
    ...
    .notion-page-block, .notion-page { scroll-margin-top: var(--minima-navbar-height) !important  }
...
def load_theme(theme_py, themes_default = {'minima' : minima}):
    theme_noext = os.path.splitext(theme_py)[0]
    if theme_noext in themes_default:
        theme = themes_default[theme_noext]
    else:
        try:
            theme = importlib.import_module(theme_noext)
        except:
...
if __name__ == '__main__':
    ...
    if args.extract_snippets and args.snippets_dir:
        os.makedirs(args.snippets_dir, exist_ok = True)
        theme = load_theme(args.theme_py)
        for k, v in getattr(theme, 'snippets_default', {}).items():
            basename = k.replace('_css', '.css').replace('_html', '.html').replace('_svg', '.svg')
            path = os.path.join(args.snippets_dir, basename)
            with open(path, 'w') as f:
                f.write(v)
            print(path)

    elif args.extract_mode in extract_mode_json or args.notion_page_id or (args.input_json and os.path.exists(args.input_json) and os.path.isfile(args.input_json)):
